<x-app-layout>
    ...
    <div class="overflow-x-auto p-6">
        <div class="max-w-2xl mx-auto p-6 bg-white rounded-md shadow-md">
            <form
                action="{{ route('job-vacancies.update', ['job_vacancy' => $vacancy->id, 'from_where' => $fromWhere]) }}"
                method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4 p-6 bg-gray-50 border-gray">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="type" id="type"
                            ...
                            value="{{ old('type', $vacancy->type) }}" placeholder="Select Type">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="category_id" id="category_id"
                            ...
                            value="{{ old('category_id', $vacancy->jobCategory->id) }}" placeholder="Select Category">
                            <option value="{{ $vacancy->jobCategory->id }}">{{ $vacancy->jobCategory->name }}</option>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea type="text" name="description" id="description" rows="4"
                            class="{{ $errors->has('description') ? 'border-red-500' : '' }}  mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ old('description', $vacancy->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="flex justify-end mt-4">
                        <button type="submit"
                            ...
                            Edit Job Vacancy</button>
                        <a href="{{ route($fromWhere == 'show' ? 'job-vacancies.show' : 'job-vacancies.index', ['job_vacancy' => $vacancy->id]) }}"
